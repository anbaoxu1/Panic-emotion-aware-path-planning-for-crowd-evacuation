...
def get_trainers(env, num_adversaries, obs_shape_n, action_shape_n, arglist):
    ...
    for i in range(1):
        actors_cur[i] = openai_actor(obs_shape_n[i], action_shape_n[i], arglist).to(arglist.device)#代表着是此网络的参数
        critics_cur[i] = openai_critic(18, 2, arglist).to(arglist.device)
        actors_tar[i] = openai_actor(obs_shape_n[i], action_shape_n[i], arglist).to(arglist.device)
        critics_tar[i] = openai_critic(18, 2, arglist).to(arglist.device)
...
def train(arglist):
    ...
    action_shape_n = [2]  # no need for stop bit
    #[5,5,5,5]tor
    # ch.mean(critic_c(obs_n_o, action_cur_o))
    ...
    for episode_gone in range(arglist.max_episode):
        ...
        for episode_cnt in range(arglist.per_episode_max_len):
            ...
            # interact with env
            new_obs_n, rew_n, done_n,fla = env.step(action_n)
            obs_road.append(new_obs_n)
            # env.render()
            """
...
if __name__ == '__main__':
    arglist = parse_args()
    obs_road=np.array(train(arglist))
    print(obs_road[:,:,:2])
