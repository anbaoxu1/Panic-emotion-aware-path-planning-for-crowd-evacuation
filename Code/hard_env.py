    ...
    def step(self,action):
        ...
        for index in range(len(self.Agent)):
            ...
            for i in range(len(self.Agent)):
                if i != index:
                    relate_p.append(self.Agent[i].p-self.Agent[index].p)
                    relate_g.append(self.Agent[i].p-self.Agent[i].g)
            for b in relate_p:
                pp.append(b[0])
                pp.append(b[1])
            for g in relate_g:
                gg.append(g[0])
                gg.append(g[1])
            xobs=np.concatenate([np.array([self.Agent[index].p[0], self.Agent[index].p[1],self.Agent[index].v[0], self.Agent[index].v[1],
                            self.Agent[index].g[0], self.Agent[index].g[1],
                    ])]+[pp]+[gg]) * 0.001
            obs.append(xobs)

        for index in range(len(self.Agent)):
            if self.collection(self.Agent[index].p[0], self.Agent[index].p[1], 30, self.Agent[index].g[0], self.Agent[index].g[1], 50):
                reward[index] += 1
                done[index] = True
        x = combinations(self.Agent, 2)
        for test in x:
            if self.col(test[0],test[1],20):
                for index in range(len(self.Agent)):
