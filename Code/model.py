...
class openai_critic(abstract_agent):
    ...
    def forward(self, obs_input, action_input):
        ...
        x1 = self.LReLU(self.linear_c2(x_cat))
        x = self.LReLU(self.linear_c3(x1))
        value = self.linear_c(x)
        return value
...
class openai_actor(abstract_agent):
    def __init__(self, num_inputs, action_size, args):
        ...
        self.linear_a3 = nn.Linear(args.num_units_openai, args.num_units_openai)
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_uniform_(self.linear_a.weight, gain=nn.init.calculate_gain('tanh'))
    ...
    def forward(self, input, model_original_out=False):
        ...
        policy = F.tanh(model_out - torch.log(-torch.log(u)))
        #policy=F.hardtanh(model_out,-1,1)
