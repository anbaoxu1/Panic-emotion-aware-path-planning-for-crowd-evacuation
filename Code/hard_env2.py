    ...
    def step(self,action):
        ...
        for index in range(len(self.Agent)):
            if self.dist1(self.Agent[index]):
                #self.Agent[index].p=np.array([8000,8000])
                reward[index] += 1
                done[index] = True
        x = combinations(self.Agent, 2)
        for test in x:
...
#重新写reset环境，使得智能体出现在不同位置，并且不会与障碍物重叠
    def reset(self):
        ...
        """
        self.Agent[0].setatt(np.array([200, 400]), [0, 0], np.array([600, 400]))
        self.Agent[1].setatt(np.array([600, 400]), [0, 0], np.array([200, 400]))
        self.Agent[2].setatt(np.array([400, 200]), [0, 0], np.array([400, 600]))
        self.Agent[3].setatt(np.array([400, 600]), [0, 0], np.array([400, 200]))
        self.Agent[4].setatt(np.array([600, 200]), [0, 0], np.array([200, 600]))
        self.Agent[5].setatt(np.array([200, 600]), [0, 0], np.array([600, 200]))
        self.Agent[6].setatt(np.array([200, 200]), [0, 0], np.array([600, 600]))
        self.Agent[7].setatt(np.array([600, 600]), [0, 0], np.array([200, 200]))
        self.Agent[8].setatt(np.array([100, 700]), [0, 0], np.array([700, 100]))
        self.Agent[9].setatt(np.array([700, 100]), [0, 0], np.array([100, 700]))
        self.Agent[10].setatt(np.array([300, 700]), [0, 0], np.array([700, 300]))
    ...
    def test_RVO(self):
        ...
        # 计算首选速度
        V_des,done = compute_V_des(P, G, V_max)
        if all(done):
            print(self.num)
            self.num+=1
            self.reset()
