...
def train(arglist):
    ...
    print(x)
    print('=============================')
    print('this is four agent')
    print('=1 Env {} is right ...'.format(arglist.scenario_name))
    print('=============================')
    obs_shape_n = []

    action_shape_n = [] # no need for stop bit
    """step2: create agents"""
    for i in range(n):
        obs_shape_n.append(x)
        action_shape_n.append(2) # no need for stop bit
...
if __name__ == '__main__':
    arglist = parse_args()
    x,y=train(arglist)
    np.savetxt('x.txt', x, fmt='%0.2f')
    np.savetxt('y.txt', y, fmt='%0.2f')
