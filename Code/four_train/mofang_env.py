...
class  ArmEnv(object):
    ...
    def step(self,action):
        ...
        for index in range(len(action)):
            ...
            getindex=self.compare(self.list,pre_pos[index])
            if(getindex==len(self.list)-1):
                continue
            else:
                sp1=self.list[getindex][1:3]
                getindex+=1
                sp2=self.list[getindex][1:3]
                a=(np.array(sp2)-np.array(sp1))/self.dt

            temp=self.computerdis(a,action[index])

            lll=math.exp(-temp) * self.W1
            reward[index]+=lll
        """
        """
    ...
    def dist2(self, a1, a2,a3,a4):
        return np.sqrt((a1-a3)**2+(a2-a4)**2)
    def dist1(self,a1):
    ...
    def compare(self,list,pos):
        #返回一个index索引
        op=[]
        #op为列表？
        for i in range(len(list)):
            #kk=abs(float(list[i][0])-pos[0])
            k=self.dist2(float(list[i][1]),float(list[i][2]),pos[0],pos[1])
            op.append(k)

        return op.index(min(op))
    ...
    def reset(self):
        ...
        for index in range(len(self.Agent)):
            ...
            if a==0:
                ...
                y = np.array([575,800])
            self.Agent[index].setatt(np.array([547.54,76.27]),np.array([0,0]),[396.64,311.78])
        """
