...
class Viewer(pyglet.window.Window):
    ...
    def __init__(self,agentinfo,ox):
        ...
        for index in range(len(self.o)):
            self.obs.append(self.batch.add(
                4, pyglet.gl.GL_QUADS, None,  # 4 corners
                ('v2f', [self.o[index].x - self.obsmax, self.o[index].y - self.obsmax,
                         self.o[index].x - self.obsmax, self.o[index].y + self.obsmax,
                         self.o[index].x + self.obsmax, self.o[index].y + self.obsmax,
                         self.o[index].x + self.obsmax, self.o[index].y - self.obsmax]),
                ('c3B', (0, 0, 255) * 4), ))
    ...
    def _update_arm(self,p,g,o):
        ...
        for index in range(len(o)):
            a = (o[index].x - self.obsmax, o[index].y - self.obsmax)
            b = (o[index].x - self.obsmax, o[index].y + self.obsmax)
            c = (o[index].x + self.obsmax, o[index].y + self.obsmax)
            d = (o[index].x + self.obsmax, o[index].y - self.obsmax)
            self.obs[index].vertices = np.concatenate((a,b,c,d))
    def render(self,p,g,o):
        self._update_arm(p,g,o)
        self.switch_to()
...
class  ArmEnv(object):
    ...
    def __init__(self,n):
        ...
        self.Agent=[]
        self.Obstacle=[]
        #self.temmp=np.array([[708,287],[309,471],[488,449],[317,71],[594,667],[93,662],[69,437],
        #                    [392,575],[390,170],[299,131]])
        #self.temmp = np.array([[645, 657], [71, 143], [362, 331], [441, 383], [497, 377], [178, 491], [517, 193],
        #
        #self.temmp = np.array([[195, 186], [138, 403], [524, 459], [491, 429], [351, 635], [149, 368], [109, 640],
        #                       [710, 152], [315, 303], [179, 300]])
        for i in range(n):
        ...
        for i in range(10):
            ...
            x = float(np.random.randint(50, 750, 1))
            y = float(np.random.randint(50, 750, 1))

            o=Obstacle(x,y,30,30)
    ...
    def computerdis2(self,x1,y1,x2,y2):
        return math.sqrt(np.square(x1-y2)+np.square(y1-y2))
    ...
    def step(self,action):
        ...
        """
        计算离当前智能体最近的3个智能体的状态（距离目标位置，障碍物相对距离，相对速度）
        """
        for index in range(len(self.Agent)):
            relate_p = []
            relate_g = []
            ...
            for i in range(len(self.Obstacle)):
                    #x=self.Agent[index].p[0]- self.Obstacle[i].x
                    #y=self.Agent[index].p[1]- self.Obstacle[i].y
                    x=self.computerdis2(self.Agent[index].p[0],self.Agent[index].p[1],self.Obstacle[i].x,self.Obstacle[i].y)
                    relate_p.append(np.array([x]))
            for b in relate_p:
                pp.append(b[0])
                #pp.append(b[1])
            for g in relate_g:
    ...
    def get_obs(self):
        ...
        for index in range(len(self.Agent)):
            ...
            for i in range(len(self.Obstacle)):
                ...
                x = self.computerdis2(self.Agent[index].p[0], self.Agent[index].p[1], self.Obstacle[i].x,
                                      self.Obstacle[i].y)
                relate_p.append(np.array([x]))
...
#重新写reset环境，使得智能体出现在不同位置，并且不会与障碍物重叠
    ...
    def render(self):
        ...
        for index in range(len(self.Obstacle)):
            ...
            o.append(self.Obstacle[index])
        self.viewer.render(p,g,o)  # 使用 Viewer 中的 render 功能
