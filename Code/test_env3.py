...
class  ArmEnv(object):
    ...
    def step(self,action):
        ...
        rvo_pos = np.zeros((self.n, 2))
        for index in range(len(action)):
            pre_pos[index] = copy.deepcopy(self.Agent[index].p)
            rvo_pos[index][0]=self.Agent[index].p[0] + action[index][0] * self.dt
            rvo_pos[index][1]=self.Agent[index].p[1] + action[index][1] * self.dt
        ...
        # 计算RVO速度
            ...
            for index in range(len(self.Agent)):
                self.Agent[index].p[0] += V[index][0] * self.dt1
                self.Agent[index].p[1] += V[index][1] * self.dt1
...
#重新写reset环境，使得智能体出现在不同位置，并且不会与障碍物重叠
    def reset(self):
        ...
        for i in range(len(self.Agent)):
            if i % 2 == 0:
                x = float(np.random.randint(50, 380, 1))
                t = float(np.random.randint(450, 750, 1))
                a = np.random.randint(0, 2, 1)
                if a == 0:
                    y = np.array([780, 360])
                else:
                    y = np.array([575, 800])
                self.Agent[i].setatt(np.array([x,t]), np.array([0, 0]), y)
            else:
                x = float(np.random.randint(50, 350, 1))
                t = float(np.random.randint(50, 300, 1))
