...
class  ArmEnv(object):
    ...
    def __init__(self,n):
        #初始化Agent
        self.n=n
        self.agent1 = Agent([200 ,100], [0 ,0], [260,400],10,50)
        self.agent2 = Agent([300, 100], [0, 0], [260,400],10,50)
        self.agent3 = Agent([400, 100], [0, 0], [260, 400], 10, 50)
        self.agent4 = Agent([500, 100], [0, 0], [260, 400], 10, 50)
        self.agent5 = Agent([700, 700], [0, 0], [260, 400], 10, 50)
        self.agent6 = Agent([400, 700], [0, 0], [260, 400], 10, 50)
        self.agent7 = Agent([300, 700], [0, 0], [260, 400], 10, 50)
        self.agent8 = Agent([200, 700], [0, 0], [260, 400], 10, 50)
    ...
    def step(self,action):
        ...
        r1 += math.exp(-dists1)*0.7
        r2 += math.exp(-dists2)*0.7
        r3 += math.exp(-dists3) * 0.7
        r4 += math.exp(-dists4) * 0.7
        r5 += math.exp(-dists5) * 0.7
        r6 += math.exp(-dists6) * 0.7
        r7 += math.exp(-dists7) * 0.7
        r8 += math.exp(-dists8) * 0.7
        #更新智能体状态,获得新obs
...
#重新写reset环境，使得智能体出现在不同位置，并且不会与障碍物重叠
    def reset(self):
        obs=[]
        self.agent1.setatt(np.random.randint(100, 700, size=2),  [0,0],  [260,400],10,50)
        self.agent2.setatt(np.random.randint(100, 700, size=2),  [0, 0], [260,400], 10, 50)
        self.agent3.setatt(np.random.randint(100, 700, size=2), [0, 0], [260, 400], 10, 50)
        self.agent4.setatt(np.random.randint(100, 700, size=2), [0, 0], [260, 400], 10, 50)
        self.agent5.setatt(np.random.randint(100, 700, size=2), [0, 0], [260, 400], 10, 50)
        self.agent6.setatt(np.random.randint(100, 700, size=2), [0, 0], [260, 400], 10, 50)
        self.agent7.setatt(np.random.randint(100, 700, size=2), [0, 0], [260, 400], 10, 50)
        self.agent8.setatt(np.random.randint(100, 700, size=2), [0, 0], [260, 400], 10, 50)
        aobs = np.array(
...
if __name__ == '__main__':
    ...
    def test(sx, sy, dx, dy, cx, cy, r):
        ...
        vx = cx - sx
        vy = cy - sy
        z = math.sqrt(vx * vx + vy * vy)  # 圆心距离

        vx /= z
        vy /= z  # 向量归一化
        w = (dx * vx + dy * vy) * z  # 射线到圆心垂直距离
        d = z * z - w * w
        rr = r * r
        if d < rr:
            z = w - math.sqrt(rr - d)  # 射线到圆环距离
        else:
            z = -1.0
        print(z)
    ...
    for e in range(20):
        test(0.0, 0.0, math.cos(e * 0.1), math.sin(e * 0.1), 2.0, 0.0, 1.0)
        """
